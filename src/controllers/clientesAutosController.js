...
const CrearCliente = async (req, res) => {
    try {
        ...
        const cliente = await ClientesAutos.findOne({email})
        if (cliente) {
            return res.status(400).json({ msg: 'Cliente ya registrado' })
        }
        const nuevoCliente = new ClientesAutos({
            cedula,
            nombre,
            apellido,
            email,
...
const ListarClientes = async (req, res) => {
    try {
        const clientes = await ClientesAutos.find()
...
const DetalleCliente = async (req, res) => {
    try {
        ...
        const cliente = await ClientesAutos.findById(id)
        if (!cliente) {
...
const ActualizarCliente = async (req, res) => {
    try {
        ...
        await ClientesAutos.findByIdAndUpdate(id, { cedula, nombre, apellido, email, telefono, direccion, ciudad, fechaNacimiento })
...
const EliminarCliente = async (req, res) => {
    try {
        ...
        await ClientesAutos.findByIdAndDelete(id)
